...
class WGL_Image_Comparison extends Widget_Base
{
    ...
    {
        return esc_html__('WGL Image Comparison', 'burido-core');
    }
    ...
    protected function register_controls()
    {
        ...
        $this->add_control(
            'before_image',
            [
                'label' => esc_html__('Before Image', 'burido-core'),
        ...
        $this->add_control(
            'after_image',
            [
                'label' => esc_html__('After Image', 'burido-core'),
                'type' => Controls_Manager::MEDIA,
			    'dynamic' => ['active' => true],
                'label_block' => true,
                'default' => [
        ...
        $this->start_controls_section(
			'section_style',
			[
				'label' => esc_html__('Slider Bar Styles', 'burido-core'),
        ...
        $this->add_control(
			'slider',
			[
				'label' => esc_html__('Slider Bar', 'burido-core'),
        ...
        $this->add_control(
            'slider_color-dynamic',
            [
                'label' => esc_html__('Color - Dynamic', 'burido-core'),
                'type' => Controls_Manager::COLOR,
                'separator' => 'after',
                'dynamic' => ['active' => true],
                'condition' => [ 'slider_color!' => '' ],
                'selectors' => [
                    'body.wgl_dynamic_colors-active {{WRAPPER}} .cocoen-drag::before,
					 body.wgl_dynamic_colors-active {{WRAPPER}} .cocoen-drag::after' => 'color: {{VALUE}};',
                ],
            ]
        );
        $this->add_control(
        ...
        $this->add_control(
            ...
            [
                ...
                'selectors' => [
                    'body.wgl_dynamic_colors-active {{WRAPPER}} .cocoen-drag::before,
					 body.wgl_dynamic_colors-active {{WRAPPER}} .cocoen-drag::after' => 'background-color: {{VALUE}};',
                ],
            ]
        );
        $this->end_controls_section();
        ...
        if (!!$_s['slider_color']){
            $styles = '.elementor-element-' . esc_attr( $this->get_id() ) . ' .wgl-image_comparison.cocoen .cocoen-drag::before,
                       .elementor-element-' . esc_attr( $this->get_id() ) . ' .wgl-image_comparison.cocoen .cocoen-drag::after{
                        background-image: url(\'data:image/svg+xml; utf8, '.wgl_dynamic_styles()->bg_caret($_s['slider_color'] ? esc_attr($_s['slider_color']) : WGL_Globals::get_primary_color()).'\'); }';
        }
