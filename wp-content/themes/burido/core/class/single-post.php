...
if (!class_exists('Burido_Single_Post')) {
    /**
     * @package burido\core\class
     * @author WebGeniusLab <webgeniuslab@gmail.com>
     * @since 1.0.0
     */
    class Burido_Single_Post
    {
	    /**
	     * @var Burido_Single_Post
	     */
        ...
        public function featured_gallery($image_size)
        {
            ...
            echo '<div class="wgl-carousel_wrapper">';
                ...
                echo '<div class="wgl-navigation_wrapper">';
                    echo '<button class="elementor-swiper-button elementor-swiper-button-prev" data-carousel="' . esc_attr($carousel_item) . '"><i class="wgl-svg-icon">'.wgl_dynamic_styles()->wgl_theme_svg()['arrow-down-right'].'</i></button>';
                    echo '<button class="elementor-swiper-button elementor-swiper-button-next" data-carousel="' . esc_attr($carousel_item) . '"><i class="wgl-svg-icon">'.wgl_dynamic_styles()->wgl_theme_svg()['arrow-down-right'].'</i></button>';
                echo '</div>';
            echo '</div>';
                ...
                echo (true == $wrapper) ? '<div class="meta-data">' : '';
                    ...
                    if ($this->show_meta_comments) {
                        ...
                        echo '<span class="comments_post">',
                            '<a href="', esc_url(get_comments_link()), '" title="', esc_attr__('Leave a reply', 'burido'), '">',
                                esc_html($comments_num),
                                ' ',
                                '<span>', esc_html(_n('Comment', 'Comments', $comments_num, 'burido')), '</span>',
                            '</a>',
                        '</span>';
                    }
        ...
        public function render_author_info()
        {
            ...
            if (function_exists('wgl_user_social_medias_arr')) {
                foreach (wgl_user_social_medias_arr() as $social => $value) if (get_the_author_meta($social)) {
                    switch ($social) {
                        case 'twitter':
                            $social_html = esc_html__('Twitter', 'burido');
                            break;
                        case 'facebook-f':
                            $social_html = esc_html__('Facebook', 'burido');
                            break;
                        case 'instagram':
                            $social_html = esc_html__('Instagram', 'burido');
                            break;
                        case 'linkedin-in':
                            $social_html = esc_html__('Linkedin', 'burido');
                            break;
                        case 'telegram-plane':
                            $social_html = esc_html__('Telegram', 'burido');
                            break;
                        default:
                            $social_html = '';
                            break;
