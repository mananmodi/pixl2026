        ...
        */
            ...
            <div class="notice notice-error is-dismissible wgl-notice">
                <p>
                    <?php echo wp_get_theme()->get('Name');
                        echo sprintf( esc_html__( ' Theme is not activated! Please activate your copy of the %s Theme.', 'burido'), wp_get_theme()->get('Name') );
                        ?>
                </p>
                <p>
                    <strong class="activate_error"><?php esc_html_e( 'Please activate the theme!', 'burido' ); ?></strong> -
                    <a href="<?php echo esc_url(( $page_purchase )); ?>">
                        <?php esc_html_e( 'Activate Now','burido' ); ?>
                    </a>
                </p>
            </div>
            ...
            <div class="notice notice-warning is-dismissible">
                <p class="support-until_text">
                    <?php
                        ...
                        if($now > $until_date){
                            echo sprintf( wp_kses( __( '<strong>Your support period expired. <a href="%s">You can prolong support</a> services as you need.</strong>', 'burido' ), $allowed_html), esc_url($item_url));
                        }elseif($now < $until_date && $interval->days < 30){
                            echo sprintf( wp_kses( __( '<strong>Your support period will be expired after %d days. <a href="%s">You can prolong support</a> services as you need.</strong>', 'burido' ), $allowed_html), $interval->days, esc_url($item_url));
                        }
                ...
                <p>
                    ...
                    ?>
                    <a href="<?php echo esc_url($remind_url); ?>" class="button button-primary"><?php echo esc_html__( 'Remind me later', 'burido' ); ?></a>&nbsp;
                    <a href="<?php echo esc_url($delete_url); ?>" class="button button-primary"><?php echo esc_html__( 'Hide and disable further reminders', 'burido' ); ?></a>
                </p>
            </div>
        ...
        public function purchase_activation(){
            ...
            if ( ! isset( $_POST['email'] ) ||  ! isset( $_POST['purchase_code'] ) || ! isset( $_POST['security'] ) || ! wp_verify_nonce( $_POST['security'], 'purchase-activation' ) ) {
                $output['error'] = 1;
                $output['message'] = esc_html__( 'Please enter a valid field', 'burido' );
                ...
                if( ! is_email( $email ) ){
                    $output['error'] = 1;
                    $output['message'] = esc_html__( 'Please enter a valid email address.', 'burido' );
                    echo json_encode( $output );
                    wp_die();
                }
                ...
                if(empty($purchase)){
                    $output['error'] = 1;
                    $output['message'] = esc_html__( 'Purchase code is empty ', 'burido' );
                    echo json_encode( $output );
                    wp_die();
                }
                ...
                if( $return !== false ){
                    ...
                    if(isset($result['success']) && !empty($result['success'])){
                        ...
                        $output['message'] =  esc_html__( 'Thank you, your license has been validated', 'burido' );
                        ...
                        $output['message'] .= '<span class="register_purchase--title">'.esc_html__( 'This purchase code has been registered.', 'burido' ).'</span><br/>';
                        $output['message'] .= esc_html__( 'Please go to your previous working environment and deactivate the purchase code to use it again (WP dashboard -> WebGeniusLab -> Activate Theme -> click on the button "Deactivate" ).', 'burido' ).'<br/><br />';
        ...
        public function reset_purchase(){
            ...
            if ( ! isset( $_POST['email'] ) || ! isset( $_POST['accout'] ) ||  ! isset( $_POST['purchase_code'] ) || ! isset( $_POST['security'] ) || ! wp_verify_nonce( $_POST['security'], 'purchase-reset-activation' ) ) {
                ...
                if(empty($accout)){
                    $output['error'] = 1;
                    $output['message'] = esc_html__( 'Your Envato Account is empty ', 'burido' );
                    echo json_encode( $output );
                    wp_die();
                }
                ...
                if (false === $response) {
                    ...
                    if( $return !== false ){
                        ...
                        if(isset($result['success']) && !empty($result['success'])){
                            $output['success'] = 1;
                            $output['error'] = 0;
                            $output['message'] =  esc_html__( 'Your Request Has been Sent', 'burido' );
                    ...
                    $output['success'] = '';
                    $output['message'] = esc_html__( 'The request has been sent before. You will get an email when it is reviewed.', 'burido' );
                    $output['error'] = 1;
                    echo json_encode( $output );
                }
            }
