...
if (!class_exists('Burido_Woocoommerce') ) {
    /**
     * Burido Woocommerce
     *
     *
     * @package burido\woocoomerce
     * @author WebGeniusLab <webgeniuslab@gmail.com>
     * @since 1.0.0
     */
    class Burido_Woocoommerce
    {
        private $row_class;
        ...
        function review_comments_meta_info($comment)
        {
            ...
            if ('0' === $comment->comment_approved) { ?>
                <em class="woocommerce-review__awaiting-approval">
                    <?php esc_html_e('Your review is awaiting approval', 'burido'); ?>
                </em>

            <?php } else { ?>
                <div class="comment_author_says"><?php
	                echo esc_html__('By ', 'burido');
                ...
                if ('yes' === get_option( 'woocommerce_review_rating_verification_label' ) && $verified) {
                    echo '<em class="woocommerce-review__verified verified">(' . esc_attr__('verified owner', 'burido') . ')</em> ';
                }
                ?>
        ...
        function wgl_filter_comments($comment_form)
        {
            ...
            $comment_form = [
                'title_reply' => have_comments() ? esc_html__('Your Comment', 'burido') : sprintf( esc_html__('Be the first to review &ldquo;%s&rdquo;', 'burido'), get_the_title() ),
                'title_reply_to' => esc_html__( 'Leave a reply to %s', 'burido' ),
                'title_reply_before' => '<h5 id="reply-title" class="comment-reply-title">',
                'title_reply_after' => '</h5>',
                'fields' => [
                    'author' => '<p class="comment-form-author"><label for="author"></label><input class="form_field" id="author" name="author" type="text" placeholder="'.esc_attr__('Your Name *', 'burido').'" value="' . esc_attr( $commenter['comment_author'] ) . '" size="30" '.$html_req.' /></p>',
                    'email' => '<p class="comment-form-email "><label for="email"></label><input class="form_field" id="email" name="email" type="email" placeholder="'.esc_attr__('Your Email *', 'burido').'" value="' . esc_attr( $commenter['comment_author_email'] ) . '" size="30" '.$html_req.' /></p>',
                ],
                'label_submit' => esc_html__('Add Review', 'burido'),
            ...
            if ($account_page_url = wc_get_page_permalink('myaccount')) {
                ...
                $comment_form['must_log_in'] = '<p class="must-log-in">' . sprintf( wp_kses( __( 'You must be <a href="%s">logged in</a> to post a review.', 'burido' ), $allowed_html), esc_url( $account_page_url ) ) . '</p>';
            }

            if (get_option('woocommerce_enable_review_rating') === 'yes') {
                $comment_form['comment_field'] = '<div class="comment-form-rating"><label for="rating">' . esc_html__('Your rating for this product', 'burido') . '</label><select name="rating" id="rating" required>
                <option value="">' . esc_html__('Rate&hellip;', 'burido') . '</option>
                <option value="5">' . esc_html__('Perfect', 'burido') . '</option>
                <option value="4">' . esc_html__('Good', 'burido') . '</option>
                <option value="3">' . esc_html__('Average', 'burido') . '</option>
                <option value="2">' . esc_html__('Not that bad', 'burido') . '</option>
                <option value="1">' . esc_html__('Very poor', 'burido') . '</option>
                </select></div>';
            }

            $comment_form['comment_field'] .= '<p class="comment-form-comment"><label for="comment"></label><textarea id="comment" name="comment" cols="45" rows="8" placeholder="'.esc_attr__('Your Comment', 'burido').'" required></textarea></p>';
        ...
        */
        ...
        {
            $array['over_image'] = esc_html__('Over the Image', 'burido');
            return $array;
        }
        ...
        {
            return '<span class="onsale">' . esc_html__( 'Sale', 'burido' ) . '</span>';
        }
	    ...
	    {
		    if (is_single()){
			    ?><span class="quantity_title"><?php esc_html_e('Quantity', 'burido'); ?></span><?php
		    }
	    }
	    ...
	    {
		    return '<span>'.esc_html__( 'Remove from list', 'burido' ).'</span>';
	    }
        ...
        public function header_add_to_cart_fragment($fragments)
        {
            ...
            echo '<span class="woo_mini-count">',
                '<i class="wgl-icon-cart">',wgl_dynamic_styles()->wgl_theme_svg()['cart'],'</i>',
        ...
        {
            echo '<strong>' . esc_html__( 'Subtotal', 'burido' ) . '</strong> ' . WC()->cart->get_cart_subtotal(); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
        }
        ...
        public function wgl_sorting_filter_button()
        {
            ...
            if (!!WGL_Framework::get_option('shop_filters_switcher') && is_active_sidebar($sb_data['id'])) {
                echo '<button class="wgl-filter-button">' . esc_html__('Filters', 'burido') . '</button>';
            }
        }
                    ...
                    <div class="wgl-filter-products">
                        ...
                        <?php if (!!WGL_Framework::get_option('shop_filters_sidebar_reset_switcher')){ ?>
                            <span class="braapf_unselect_all wgl-reset-filter bapf_reset bapf_rst_nofltr bapf_rst_sel">
                                <?php esc_html_e( 'Reset Filters', 'burido' ); ?>
                            </span>
                        <?php } ?>
                    </div><?php
                }
        ...
        public function wgl_filter_pagination()
        {
            ...
            return [ // WPCS: XSS ok.
                ...
                'total' => $total,
                'prev_text' => '<i class="wgl_pagination_prev wgl-svg-icon">'.wgl_dynamic_styles()->wgl_theme_svg()['arrow-theme'].'</i>',
                'next_text' => '<i class="wgl_pagination_next wgl-svg-icon">'.wgl_dynamic_styles()->wgl_theme_svg()['arrow-theme'].'</i>',
                'type' => 'list',
                'end_size' => 1,
                'mid_size' => 2,
            ];
        ...
        public function woo_rename_tabs( $tabs ) {
            if (isset($tabs['reviews'])) {
                ...
                $tabs['reviews']['title'] = esc_html__( 'Reviews', 'burido' ) . (0 !== $count ? ' <span class="count">' . esc_html($count) . '</span>' : '');
            }

	        if (isset($tabs['additional_information'])) {
		        $tabs['additional_information']['title'] = esc_html__( 'Information', 'burido' );
	        }
	    ...
	    public function wgl_checkout_fields($fields){
		    if ( isset($fields['billing']) ) {
			    ...
			    $fields['billing']['billing_shipping_title'] = [
				    'label' => esc_html__( 'Delivery information', 'burido' ),
				    'type' => 'hidden',
        ...
        public function wgl_show_brand() {
            ...
            if ( is_singular( 'product' ) ) {
                ...
                echo wc_get_brands( $post->ID, ', ', ' <span class="posted_in">' . sprintf( '<span class="title">'._n( '%s', '%s', $brand_count, 'burido' ).'</span>', $labels->singular_name, $labels->name ), '</span>' ); // phpcs:ignore WordPress.Security.EscapeOutput
            }
        }
    }
}
...
new Burido_Woocoommerce();

if (!function_exists('burido_woocommerce_breadcrumb')) {
	/**
	 ...
	 * @return string
	 */
    function burido_woocommerce_breadcrumb($args = [])
    {
        $args = wp_parse_args($args, apply_filters('woocommerce_breadcrumb_defaults', [
            ...
            'before' => '',
            'after' => '',
            'home' => esc_html_x('Home', 'breadcrumb', 'burido'),
        ]));
