	...
	class WGL_Installer_Plugins {
		...
		public function deactivate_plugin() {
			...
			if ( ! $plugins ) {
				wp_send_json(
					[
						'message' => esc_html__( 'Plugins list is empty.', 'burido' ),
						'status'  => 'error',
					]
				);
			}
			...
			if ( ! current_user_can( 'manage_options' ) ) {
				wp_send_json(
					[
						'message' => esc_html__( 'You not have access.', 'burido' ),
						'status'  => 'error',
					]
				);
			}
			...
			if ( is_multisite() && $this->plugin_activation_for_network_check( $plugins[ $_POST['wgl_plugin'] ]['file_path'] ) ) { 
				wp_send_json(
					[
						'message' => esc_html__( 'You cannot deactivate the plugin on a multisite.', 'burido' ),
						'status'  => 'error',
					]
				);
			}
		...
		public function get_plugins() {
			...
			$plugins = $this->reinit_position_plugins($plugins, 'learnpress');
		...
		public function get_action_text( $status ) {
			switch ( $status ) {
				case 'install':
					return esc_html__( 'Install', 'burido' );
				case 'update':
					return esc_html__( 'Update', 'burido' );
				case 'activate':
					return esc_html__( 'Activate', 'burido' );
				default:
					return esc_html__( 'Deactivate', 'burido' );
			}
		}
	}
