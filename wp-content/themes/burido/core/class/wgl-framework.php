    ...
    class WGL_Framework
    {
        ...
        public static function get_option(String $name)
        {
            if (class_exists('Redux')) {
                ...
                if (!empty($GLOBALS['burido_set'])) {
                    $theme_options = $GLOBALS['burido_set'];
                } else {
                    $theme_options = get_option('burido_set');
                }
            }

            if (empty($theme_options)) {
                $theme_options = get_option('burido_default_options');
            }
        ...
        public static function bg_body_render() {
            ...
            if (class_exists('RWMB_Loader') && !is_archive() && !is_search()) {
                if ('on' === rwmb_meta('mb_body_switch')) {
                    ...
                    $mb_dark_image = rwmb_meta('mb_dark-body_color_bg');
                }
            }
            ...
            $redux_dark_image = self::get_option('dark-body_color_bg');
            ...
            // Dark
            $dark_color = !empty($mb_dark_image) ? $mb_dark_image : $redux_dark_image ?? '';
            $style['color'] .= $dark_color ? '--dark-body-background-color: ' . esc_attr($dark_color) . ';' : '';
            $style['color'] .= '--dark-body-background-rgb-color: ' . ( $dark_color ? esc_attr(WGL_Framework::hex_to_rgb($dark_color)) : '4,1,28' ) . ';';
        ...
        private static function _checkHex($hex)
        {
            ...
            if (strlen($color) == 3) {
                ...
                throw new Exception( esc_html__('HEX color needs to be 6 or 3 digits long', 'burido') );
            }
        ...
        public static function posted_meta_on()
        {
            ...
            printf(
                '<span><time class="entry-date published" datetime="%1$s">%2$s</time></span><span>' . esc_html__('Published in', 'burido') . ' <a href="%3$s" rel="gallery">%4$s</a></span>',
            ...
            if ($metadata) {
                printf(
                    ...
                    esc_html_x('Full size', 'Used before full size attachment link.', 'burido'),
            ...
            edit_post_link(
                ...
                sprintf(
                    wp_kses(__('Edit<span class="screen-reader-text"> "%s"</span>', 'burido'), $kses_allowed_html),
                    get_the_title()
                ),
