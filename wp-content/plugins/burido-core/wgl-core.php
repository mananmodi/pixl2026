...
class Burido_CorePlugin
{
    ...
    public function check_wgl_extensions_installed(){
        if (
            ...
            add_action( 'admin_notices', 'Burido_CorePlugin::wgl_plugin_notice');
        }
    }
    ...
    public function check_wgl_theme_installed()
    {
        if (
            ...
            add_action('admin_notices', 'Burido_CorePlugin::wgl_theme_notice');
    ...
    {
        ?><div class="error"><p><?php echo esc_html__('WGL Extensions plugin is required for Burido Core plugin to work properly. Please activate WGL Extensions to use Burido Core.', 'burido-core');?></p></div><?php
    }

    public static function wgl_theme_notice()
    {
        ?><div class="error"><p><?php echo esc_html__('Burido Core plugin compatible with Burido theme only!.', 'burido-core');?></p></div><?php
    }
...
new Burido_CorePlugin();

register_activation_hook(__FILE__, ['Burido_CorePlugin', 'activation_check']);
 ...
 */
function deactivate_burido_core()
{
    require_once plugin_dir_path(__FILE__) . 'includes/class-wgl-core-deactivator.php';
    Burido_Core_Deactivator::deactivate();
}

register_activation_hook(__FILE__, 'activate_burido_core');
register_deactivation_hook(__FILE__, 'deactivate_burido_core');
 ...
 */
 ...
 if (!function_exists( 'run_burido_core'))
{
    function run_burido_core()
    {
        (new Burido_Core())->run();
    }

    add_action('wgl_extensions_plugin_loader', 'run_burido_core');
}
