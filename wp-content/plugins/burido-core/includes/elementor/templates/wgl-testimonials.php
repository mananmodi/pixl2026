    ...
    class WGL_Testimonials
    {
        ...
        public function render($self, $attributes)
        {
            ...
            foreach ($items as $index=>$item) {
                ...
                $name_html = $title_html = $subtitle_html = $icon_html__block = $icon_html__content = '';
                $cursor_data = '';
                ...
                if (isset($item['cursor_tooltip']) && '' != $item['cursor_tooltip']) {
                    ...
                    $cursor_data = $cursor->build($self, $item, $item['_id']);
                }
                ...
                if (isset($title_icon_enabled) && !!$title_icon_enabled){
                    if ('content' === $title_icon_display){
                        $icon_html__content = '<span class="item__icon"></span>';
                    } else if ('block' === $title_icon_display){
                ...
                $quote_html = !empty($quote) ? '<' . esc_attr($quote_tag) . ' class="item__quote">' . wp_kses($quote, self::get_kses_allowed_html()) . $icon_html__content . '</' . esc_attr($quote_tag) . '>' : '';
        ...
        protected function get_rating_html(Int $value)
        {
            ...
            return '<div class="item__rating" role="img" aria-label="' . esc_attr__('Rated', 'burido-core') . ' ' . $value . ' out of ' . $max_rating . '">'
