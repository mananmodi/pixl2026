...
class WGLProductsGrid
{
    ...
    public function render($attributes = [], $self = false)
    {
        ...
        $styles = '';
        if (isset($_s['button_icon_color_idle']) && !!$_s['button_icon_color_idle']){
            $styles .= '.elementor-element-' . esc_attr( $item->get_id() ) . ' div.wgl-products div.product a.button{ ';
            $styles .= '--burido-button-loading: url(\'data:image/svg+xml; utf8, '.wgl_dynamic_styles()->wgl_button_loading($_s['button_icon_color_idle']).'\');';
            $styles .= '}';
        }
        if (isset($_s['button_icon_color_hover']) && !!$_s['button_icon_color_hover']){
            $styles .= '.elementor-element-' . esc_attr( $item->get_id() ) . ' div.wgl-products div.product a.button{ ';
            $styles .= '--burido-button-success: url(\'data:image/svg+xml; utf8, '.wgl_dynamic_styles()->wgl_button_success($_s['button_icon_color_hover']).'\');';
            $styles .= '}';
            $styles .= '.elementor-element-' . esc_attr( $item->get_id() ) . ' div.wgl-products div.product a.button:hover{ ';
            $styles .= '--burido-button-loading: url(\'data:image/svg+xml; utf8, '.wgl_dynamic_styles()->wgl_button_loading($_s['button_icon_color_hover']).'\');';
            $styles .= '--burido-button-success: url(\'data:image/svg+xml; utf8, '.wgl_dynamic_styles()->wgl_button_success($_s['button_icon_color_hover']).'\');';
            $styles .= '}';
        }
    ...
    protected function _render_filter($_s)
    {
        ...
        $filter .= '<a href="#"'.$page_transitions.' data-filter=".product" class="swiper-slide active"><span class="cat_title">'.esc_html__('All', 'burido-core') .'</span>' . '<span class="filter_counter"></span></a>';
        foreach ( $cats as $cat ) {
            if ( $cat->count > 0 ) {
