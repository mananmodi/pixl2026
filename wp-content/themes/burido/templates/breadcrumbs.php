...
if (!class_exists('Burido_Breadcrumbs')) {
    /**
     * Breadcrumbs area
     *
     *
    ...
    class Burido_Breadcrumbs
    {
        ...
        {
            $this->text_home     = esc_html__('Home', 'burido'); // text for the `Home` link
            $this->text_category = esc_html__('Archive by Category "%s"', 'burido'); // text for a category page
            $this->text_search   = esc_html__('Search Results for "%s"', 'burido'); // text for a search results page
            $this->text_tag      = esc_html__('Posts Tagged "%s"', 'burido'); // text for a tag page
            $this->text_author   = esc_html__('Articles Posted by %s', 'burido'); // text for an author page
            $this->text_404      = esc_html__('Error 404', 'burido'); // text for the 404 page
            $this->text_page     = esc_html__('Page %s', 'burido'); // text 'Page N'
            $this->text_cpage    = esc_html__('Comment Page %s', 'burido'); // text `Comment Page N`
        }
        ...
        public function __construct ()
        {
            ...
            $sep            = wgl_dynamic_styles()->wgl_theme_svg()['arrow-down-right']; // separator between crumbs
            ...
            if (is_home()) {
	            ...
	            if( class_exists( 'WooCommerce' ) && is_woocommerce()
		            ...
		            $out .= burido_woocommerce_breadcrumb($args);
	            } elseif ( is_category() ) {
                ...
                // Portfolio Taxonomies Archive
                    ...
                    if (get_query_var('paged')) {
                        $out .= $sep . sprintf($link, $archive_link, esc_html__('Portfolio', 'burido')) . $sep
                            . $before
                ...
                // Custom Post Type
                    ...
                    function_exists('is_product') && is_product()
                    && function_exists('burido_woocommerce_breadcrumb')
                ) {
                    $args = [];
                    $args['delimiter'] = $sep;
                    if ($show_home_link) { $out .= $sep; }
