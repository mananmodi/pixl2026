...
add_action( 'wgl/after_main_content', 'burido_after_main_content' );

/**
 * Sequence of theme specific filters
 */

add_filter( 'wgl/header/enable', 'burido_header_enable' );

add_filter( 'wgl/page_title/enable', 'burido_page_title_enable' );

add_filter( 'wgl/footer/enable', 'burido_footer_enable' );

add_filter( 'comment_form_fields', 'burido_comment_form_fields' );
...
add_filter('tiny_mce_before_init', 'burido_tiny_mce_before_init');
add_filter('mce_buttons_2', 'burido_tiny_mce_buttons_2' );

add_filter('wp_list_categories', 'burido_categories_postcount_filter');
add_filter('woocommerce_layered_nav_term_html', 'burido_categories_postcount_filter');

add_filter('get_archives_link', 'burido_render_archive_widgets', 10, 6);

add_filter('wgl/enqueue_shortcode_css', function( $styles ) {
    global $burido_dynamic_css;
    if ( ! isset( $burido_dynamic_css[ 'style' ] ) ) {
        $burido_dynamic_css = [];
        $burido_dynamic_css['style'] = $styles;
    } else {
        $burido_dynamic_css['style'] .= $styles;
    }
});
